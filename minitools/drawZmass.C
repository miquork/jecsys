...
void drawZmass(string run = "BCDEF") {
  ...
  extraText = "Preliminary";//"Private Work";
  ...
  TF1 *f1jec = new TF1("f1jec","(([0]+[1]*log(0.01*x)"
			"+[2]*pow(log(0.01*x),2))"
			"-1)*100", 30, ptmax);
  // BCDEF fit with minitools/drawZmass.C
  f1jec->SetParameters(1.00246, 0.00214, 0.00116);//17Nov17_V10
		       //1.00279, 0.00166, 0.00112);//2016
  ...
  if (run!="BCDEF") {
    f1mzee->SetParameters(1.00246, 0.00214, 0.00116);//17Nov17_V10
    f1mzee->FixParameter(1,0.00214);//17Nov17_V10
    f1mzee->FixParameter(2,0.00116);//17Nov17_V10

    // Clean out low statistics outliers
    for (int i = gzeer2->GetN()-1; i != -1; --i) {
      if (gzeer2->GetY()[i]>2.5 || gzeer2->GetY()[i]<-1) gzeer2->RemovePoint(i);
    }
  }
  ...
  if (ispr) {
    ...
    if (run!="BCDEF")
      tex->DrawLatex(0.4,0.85,Form("#Delta^{2}M(e^{+}e^{-})"
				   " = %+1.3f #pm %1.3f%%",
				   100.*(f1mzee->GetParameter(0)-1.00246),
    ...
    if (run!="BCDEF")
      tex->DrawLatex(0.4,0.80,Form("#Delta^{2}M(#mu^{+}#mu^{-})"
				   " = %+1.3f #pm %1.3f%%",
				   100.*(f1mzmm->GetParameter(0)-0.99854),
