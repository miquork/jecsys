...
void RhoVsNPV() {
  ...
  if (true) { // Z+jet, updated to 2016 Legacy re-reco
    ...
    //TFile *fp = new TFile("rootfiles/zjet_combination_07Aug2017_Summer16_JECV6_Zmm_GH_2018-03-06.root","READ");
    TFile *fp = new TFile("rootfiles/zjet_pileupSummary_07Aug2017_Summer16_JECV6_Zmm_GH_2018-03-27.root","READ");
    assert(fp && !fp->IsZombie());

    // szpt settings
    const char* szpt = "";
    //const char* szpt = "_zpt_30_50";
    //const char* szpt = "_zpt_50_130";
    //const char* szpt = "_zpt_130_500";

    //TGraphErrors *gmn = (TGraphErrors*)fp->Get("MC_npv_vs_npumean_CHS_a20_eta_00_13_L1L2L3");
    TGraphErrors *gmn = (TGraphErrors*)fp->Get(Form("MC_npv_vs_npumean_CHS%s_eta_00_13_L1L2L3",szpt));
    assert(gmn);
    //TGraphErrors *gmr = (TGraphErrors*)fp->Get("MC_rho_vs_npumean_CHS_a20_eta_00_13_L1L2L3");
    TGraphErrors *gmr = (TGraphErrors*)fp->Get(Form("MC_rho_vs_npumean_CHS%s_eta_00_13_L1L2L3",szpt));
    assert(gmr);
    //TGraphErrors *gdn = (TGraphErrors*)fp->Get("Data_npv_vs_npumean_CHS_a20_eta_00_13_L1L2L3");
    TGraphErrors *gdn = (TGraphErrors*)fp->Get(Form("Data_npv_vs_npumean_CHS%s_eta_00_13_L1L2L3",szpt));
    assert(gdn);
    //TGraphErrors *gdr = (TGraphErrors*)fp->Get("Data_rho_vs_npumean_CHS_a20_eta_00_13_L1L2L3");
    TGraphErrors *gdr = (TGraphErrors*)fp->Get(Form("Data_rho_vs_npumean_CHS%s_eta_00_13_L1L2L3",szpt));
    assert(gdr);
  ...
  */
  ...
  map<string, int> mmcm;
  map<string, int> mdtm;
  mcol["sj"]=kBlack; mmcm["sj"]=kOpenSquare; mdtm["sj"]=kFullSquare;
  ...
  for (unsigned int i = 0; i != v.size(); ++i) {
    ...
    if ((plotMCup && v[i]!="zb") || (plotZBup && v[i]=="zb"))
  ...
  if (plotGam) leg2->AddEntry(gds["gj"],"#gamma+jet (p_{T}>40 GeV)","PL");
  if (plotZmm) leg2->AddEntry(gds["zmm"],"Z+jet (p_{T}>30 GeV)","PL");
  ...
  //c1->SaveAs("pdf/RhovsNPV_Zpt_130_500.pdf"); // szpt
