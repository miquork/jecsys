...
bool doClosure = false;//true; // in reprocess.C
...
void globalFitL3Res(double etamin = 0, double etamax = 1.3,
  ...
  */
  ...
  herr->SetLineWidth(2);
  herr->SetLineColor(doClosure ? kRed+1 : kCyan+3);
  herr->SetLineStyle(kDashed);
  herr->SetFillColor(doClosure ? kRed-9 : kCyan+1);
...
//  }
  ...
  if(njesFit==1){
    if(etamin==0.&&etamax==0.261)txtL2L3 << "{ 1 JetEta 1 JetPt 1./[0] Correction L2Relative}";
